...
static void handle_connection(struct app_ctx *app_ctx, int peer_fd) {
    ...
    while (true) {
	    ...
	    /* This is a fallthrough that cleans up local objects ahead of loop_next_err */
        ...
        else if (strcmp(json_object_get_string(operation), "Encrypt") == 0) {
            /* Decrypt uses KMS to decrypt the data passed to it in the Ciphertext
             * field and sends it back to the called*
             * TODO: This should instead send a hash of the data instead.
             */
            fail_on(client == NULL, loop_next_err, "Client not initialized");
            //struct json_object *plaintext_obj = json_object_object_get(object, "Plaintext");
            struct json_object *plaintext_obj = json_object_object_get(object, "Plaintext");
            fail_on(plaintext_obj == NULL, loop_next_err, "Message does not contain a Plaintext");
            fail_on(
                !json_object_is_type(plaintext_obj, json_type_string),
                loop_next_err,
                "Ciphertext not a base64 string");
            struct json_object *keyid_obj = json_object_object_get(object, "Keyid");
            fail_on(keyid_obj == NULL, loop_next_err, "Message does not contain a Keyid");
            fail_on(
                !json_object_is_type(keyid_obj, json_type_string),
                loop_next_err,
                "Ciphertext not a base64 string");
            ...
            struct aws_string *key_id = aws_string_new_from_c_str(app_ctx->allocator, json_object_get_string(keyid_obj));
            struct aws_byte_cursor plaintext_b64 = aws_byte_cursor_from_c_str(json_object_get_string(plaintext_obj));
            //struct aws_string key_id;
            rc = aws_base64_compute_decoded_len(&plaintext_b64, &plaintext_len);
            fail_on(rc != AWS_OP_SUCCESS, loop_next_err, "Plaintext not a base64 string");
            ...
            fail_on(rc != AWS_OP_SUCCESS, encrypt_clean_err, "Memory allocation error");
            rc = aws_base64_encode(&plaintext_encrypted_cursor, &plaintext_encrypted_b64);
            fail_on(rc != AWS_OP_SUCCESS, encrypt_clean_err, "Base64 encoding error");
            aws_byte_buf_append_null_terminator(&plaintext_encrypted_b64);

            /* Send back result. */
            rc = s_send_status(peer_fd, STATUS_OK, (const char *)plaintext_encrypted_b64.buffer);
            aws_byte_buf_clean_up(&plaintext_encrypted);
